...
class DataAugmenter:
    """Classe pour augmenter des données textuelles avec différentes méthodes"""
    ...
    def random_deletion(self, text: str, p: float = 0.1) -> str:
        ...
        # Éviter de supprimer tous les mots
        keep_prob = 1 - p
        new_words = []
        ...
        # Si tous les mots ont été supprimés, garder un mot aléatoire
        if len(new_words) == 0:
    ...
    def random_swap(self, text: str, n: int = 1) -> str:
        """Échange aléatoirement la position de n paires de mots"""
        words = text.split()
        if len(words) < 2:
            return text
    ...
    def random_insertion(self, text: str, n: int = 1) -> str:
        """Insère n mots aléatoires du texte dans d'autres positions"""
        words = text.split()
        if len(words) < 2:
            return text
            
        for _ in range(n):
            # Choisir un mot aléatoire du texte
            word_to_insert = random.choice(words)
            # Choisir une position aléatoire pour l'insertion
    ...
    def synonym_replacement(self, text: str, n: int = 1, language: str = 'fra') -> str:
        ...
        # Choisir n mots aléatoires non vides
    ...
    def _get_synonym(self, word: str, language: str = 'fra') -> Optional[str]:
        """Trouve un synonyme pour un mot donné"""
        synonyms = []
    ...
    def back_translation(self, text: str, from_lang: str = 'fr', to_lang: str = 'en') -> str:
        """Pseudo-implémentation de traduction aller-retour"""
        # Note: Une implémentation réelle utiliserait une API de traduction
        # Cette version est un placeholder qui simule l'effet
        
        # Simuler des modifications légères
        words = text.split()
        
        # Modifier ~20% des mots
        for i in range(len(words)):
            if random.random() < 0.2:
                # Ajouter/supprimer un caractère ou modifier la casse
                word = words[i]
                if len(word) > 3:
    ...
    def contextual_augmentation(self, text: str) -> str:
        """Simulation de l'augmentation contextuelle avec un modèle de langage"""
        # Note: Une implémentation réelle utiliserait un modèle pré-entraîné
        # Cette version est un placeholder qui simule l'effet
        
        words = text.split()
        
        # Remplacer ~15% des mots par des mots similaires
        for i in range(len(words)):
            if random.random() < 0.15 and len(words[i]) > 3:
                # Simuler un remplacement contextuel
                words[i] = self._simulate_contextual_replacement(words[i])
                
        return ' '.join(words)
    
    def _simulate_contextual_replacement(self, word: str) -> str:
        """Simule le remplacement contextuel d'un mot"""
        transformations = [
    ...
    def augment(self, text: str, methods: List[str] = None, num_augmentations: int = 1) -> List[str]:
        """Applique plusieurs méthodes d'augmentation au texte"""
        if methods is None:
...
def augment_dataset(texts: List[str], labels: List[str], methods: List[str] = None, 
    ...
    if balanced:
        ...
        # Augmentation uniforme sur tout le dataset
...
def evaluate_augmentation_impact(texts: List[str], labels: List[str], 
    ...
    """Évalue l'impact de l'augmentation de données sur les performances"""
    ...
    # Tester différents taux d'augmentation
